...
from ...filters import ThruCellPickerFilter, NodeCellFilter
...
class SinglePicker(AbstractPicker):
    ...
    def set_data(self):
        ...
        self.mpc_picker.SetDataSet(data['rbes'])
        ...
        if data['rbes'].GetNumberOfCells() > 0:
            self.mpc_picker.Update()
            self.mpc_picker.Modified()
    ...
    def set_picking(self, active_selections):
        ...
        self.mpc_picker.set_picking(VTK_POLY_LINE, active_selections.rbes)
...
class SinglePicker2(AbstractPicker):
    def __init__(self, model_picker):
        super(SinglePicker2, self).__init__(model_picker)

        self.node_picker = NodeCellFilter()
        self.element_picker = ThruCellPickerFilter()
        self.mpc_picker = ThruCellPickerFilter()
    ...
    def connect_signals(self):
        super(SinglePicker2, self).connect_signals()

        self.model_picker.hover_data_pipeline.SetInputConnection()
    ...
    def set_input_connections(self):
        ports = self.model_picker.get_model_ports()

        node_actor = self.model_picker.get_model_actors()['nodes']

        self.node_picker.SetInputConnection(ports['nodes'])
        self.node_picker.add_pick_list(node_actor)

        self.element_picker.SetInputConnection(ports['elements'])
        self.mpc_picker.SetInputConnection(ports['mpcs'])
    ...
    def node_pick(self, pos):
        ...
        data_set = self.node_picker.GetInput()
        ...
        _id = self.node_picker.get_cell_id()
