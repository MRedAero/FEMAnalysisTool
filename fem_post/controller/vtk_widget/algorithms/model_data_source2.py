...
class BDFDataSource(VTKPythonAlgorithmBase):
    ...
    def read_bdf(self):
        ...
        for i in xrange(ugrid.GetNumberOfCells()):
            ...
            cell_type = card_ids.GetValue(i)

            visible_insert(1)

            if cell_type == grid:
                new_value = get_global_id(i) + OFFSET_NODE
                global_id1_insert(new_value)
                set_global_id2(i, new_value)
                basic_types_insert(0)
                basic_shapes_insert(VTK_NODE)

            elif cell_type == cbeam:
                new_value = get_global_id(i) + OFFSET_ELEMENT
                global_id1_insert(new_value)
                set_global_id2(i, new_value)
                basic_types_insert(2)
                basic_shapes_insert(VTK_LINE)

            elif cell_type == ctria3:
                new_value = get_global_id(i) + OFFSET_ELEMENT
                global_id1_insert(new_value)
                set_global_id2(i, new_value)
                basic_types_insert(2)
            ...
            default_group_insert(new_value)
